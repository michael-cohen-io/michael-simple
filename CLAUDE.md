...
- `public/index.md` (the home page as Markdown, served for `/` when a request sends `Accept: text/markdown`, via the rewrite in `vercel.json`) and `public/llms.txt` are generated by `scripts/build-llms.tsx` from the same content files, before every `dev` and `build` (`bun run generate`), and are never committed.
